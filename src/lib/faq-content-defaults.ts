...
export type FaqPageContent = {
  intro: { eyebrow: string };
  items: FaqItemContent[];
  contact: FaqContactContent;
};

function normalizeAnswer(answer: string | readonly string[]): string[] {
...
function toFaqItem(
  ...
  return { id, question, answerParagraphs: normalizeAnswer(answer) };
}

export const defaultFaqPageContent: FaqPageContent = {
  intro: { eyebrow: "FREQUENTLY-ASKED QUESTIONS" },
  items: [
    toFaqItem(
      "claim-overview",
      "What is this Claim about, in plain terms?",
    ...
    toFaqItem(
      "power-of-attorney",
      "What will happen after I grant a Power of Attorney to FIPO?",
    ...
    toFaqItem(
      ...
      "What continuing responsibilities will I have?",
    ...
    toFaqItem(
      "involvement-burden",
      "How onerous will my involvement be?",
    ...
    toFaqItem("legal-basis", "What is the legal basis of the Claim?", [
      "The Claim rests on three legal foundations. First, competition law: we believe Bupa and AXA PPP have breached the Chapter I and Chapter II prohibitions of the Competition Act 1998, by (a) applying restrictive fee practices across a substantial part of the market and/or coordinating their fee schedules with each other (contrary to Section 2) and (b) individually abusing their dominant market positions to impose unfairly low fees and restrictive practices on consultants (contrary to Section 18).",
    ...
    toFaqItem(
      "patient-cost",
      "Will this make private medical care more expensive for patients?",
    ...
    toFaqItem(
      "soundings",
      "How will FIPO take soundings from members?",
    ...
    toFaqItem(
      "own-solicitor",
      "Do I need my own Solicitor to sign the documents?",
    ...
    toFaqItem(
      "unsure-insurers",
      "What if I am not sure which Insurance companies I have worked with?",
    ...
    toFaqItem(
      "separate-claim",
      "Can I also pursue a separate claim against the insurers myself?",
    ...
    toFaqItem(
      "fipo-insolvent",
      "What happens if FIPO becomes insolvent?",
    ...
    toFaqItem(
      "death",
      "What happens if I die before the litigation concludes?",
    ...
    toFaqItem(
      "timing",
      "How long will it take before I receive any payment?",
    ...
    toFaqItem("legal-team", "Who is in the Legal team?", [
    ...
    toFaqItem(
      ...
      "Are my communications with FIPO and Harcus Parker confidential?",
    ...
    toFaqItem(
      "retired",
      "I retired from private practice. Can I still join?",
    ...
    toFaqItem(
      "small-income",
      "I only have a small amount of PMI income. Is it worth joining?",
    ...
    toFaqItem("confidentiality", "Will my involvement in the Claim be kept confidential?", [
...
export function mergeFaqPageContent(
  content: Partial<FaqPageContent> | null | undefined
): FaqPageContent {
  const items =
    content?.items
      ?.map((item) => ({
        ...
        answerParagraphs: (item.answerParagraphs ?? [])
          .map((p) => p.trim())
          .filter(Boolean),
      }))
  ...
  return {
    intro: {
      eyebrow: content?.intro?.eyebrow?.trim() || defaultFaqPageContent.intro.eyebrow,
    },
    items: items.length > 0 ? items : defaultFaqPageContent.items,
    contact: {
      eyebrow: content?.contact?.eyebrow?.trim() || defaultFaqPageContent.contact.eyebrow,
      legal: {
        title:
          content?.contact?.legal?.title?.trim() ||
          defaultFaqPageContent.contact.legal.title,
        description:
          content?.contact?.legal?.description?.trim() ||
          defaultFaqPageContent.contact.legal.description,
        email:
          content?.contact?.legal?.email?.trim() ||
          defaultFaqPageContent.contact.legal.email,
      },
      admin: {
        title:
          content?.contact?.admin?.title?.trim() ||
          defaultFaqPageContent.contact.admin.title,
        description:
          content?.contact?.admin?.description?.trim() ||
          defaultFaqPageContent.contact.admin.description,
        email:
          content?.contact?.admin?.email?.trim() ||
          defaultFaqPageContent.contact.admin.email,
      },
      disclaimer:
        content?.contact?.disclaimer?.trim() || defaultFaqPageContent.contact.disclaimer,
    },
  };
}

export function slugifyFaqId(text: string) {
  return text
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "-")
    .replace(/^-|-$/g, "");
